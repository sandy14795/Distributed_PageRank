...
def main(args):
    ...
    node_adj = data_rdd.groupByKey().cache() #cache the node adjacency list RDD as it is used multiple times
    node_ranks = node_adj.mapValues(lambda x: 1.0)

    for i in range(10):
        contribs = node_adj.join(node_ranks).flatMap(lambda x: contrib(x[1][1], x[1][0]))
        contribs = contribs.reduceByKey(lambda x, y: x+y)
        node_ranks = contribs.mapValues(lambda x: 0.15 + 0.85*x).cache() #cache the node ranks RDD as it is used multiple times
